...
def eval_linear_regression(x_train, x_test, y_train, y_test):
    ...
    def compute_acc(y_test, y_test_predict, margin_1=0.5, margin_2=1.0):
...
if __name__ == "__main__":
    ...
    print(f"abstractness ranges from {min(word_to_abstractness.values())} to {max(word_to_abstractness.values())} with mean {mean(word_to_abstractness.values()):.2f}")

    x_train, x_test, y_train, y_test = get_x_y(word_to_embedding, word_to_abstractness)
